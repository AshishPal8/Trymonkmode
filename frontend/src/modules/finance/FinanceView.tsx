...
import { formatCurrency, formatDatePretty, getTodayDateString } from '@/lib/utils';
...
export function FinanceView() {
  ...
  const [selectedMonth, setSelectedMonth] = useState<string>(todayStr.slice(0, 7)); // YYYY-MM for monthly
  const [selectedYear, setSelectedYear] = useState<number>(parseInt(todayStr.slice(0, 4), 10)); // YYYY for yearly
  ...
  const [paymentMethod, setPaymentMethod] = useState<TransactionItem['paymentMethod']>('Apple Pay');
  ...
  const getCategoryIcon = (cat: TransactionItem['category']) => {
    switch (cat) {
      ...
      case 'Tech & Subscriptions': return <Tv className="w-4 h-4 text-purple-500" />;
      case 'Rent': return <Home className="w-4 h-4 text-sky-500" />;
      case 'Health': return <HeartPulse className="w-4 h-4 text-rose-500" />;
      case 'Shopping':
      default: return <ShoppingBag className="w-4 h-4 text-pink-500" />;
    }
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs sm:text-sm text-muted-foreground">
            Apple Wallet style cashflow ledger with daily, monthly, and yearly timeframes
          </p>
        </div>
      ...
      <div className="ios-card rounded-3xl p-6 sm:p-7 space-y-5">
        <div className="flex items-start justify-between">
          ...
          <span className="text-[10px] font-bold uppercase px-2.5 py-1 rounded-full bg-[#0052FF]/10 text-[#0052FF] font-mono">
            {timeframeTransactions.length} {timeframeTransactions.length === 1 ? 'Transaction' : 'Transactions'}
          </span>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="p-4 rounded-2xl cat-inbox flex items-center gap-3">
            ...
            <div>
              <span className="text-[10px] font-semibold uppercase opacity-80">
                {timeframe === 'daily' ? 'Day Spending' : timeframe === 'monthly' ? 'Month Spending' : 'Spending'}
              </span>
          ...
          <div className="p-4 rounded-2xl cat-done flex items-center gap-3">
            ...
            <div>
              <span className="text-[10px] font-semibold uppercase opacity-80">
                {timeframe === 'daily' ? 'Day Income' : timeframe === 'monthly' ? 'Month Income' : 'Income'}
              </span>
              <p className="text-sm sm:text-base font-bold font-mono">
        ...
        {displayTransactions.length === 0 ? (
          <div className="p-12 text-center rounded-3xl ios-card border border-border space-y-3">
            ...
            <div className="space-y-1">
              <h4 className="text-sm font-semibold text-foreground">No Transactions for this Period</h4>
              <p className="text-xs text-muted-foreground max-w-xs mx-auto">
                No recorded income or expenses in this timeframe. Click below to add a transaction.
              </p>
            </div>
            <Button
              type="button"
              onClick={() => {
                setTxDate(timeframe === 'daily' ? selectedDate : getTodayDateString());
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-fadeIn">
          <div className="ios-card rounded-3xl p-6 w-full max-w-md shadow-2xl space-y-4">
            <h3 className="text-base font-bold text-card-foreground">Add Transaction</h3>
            ...
            <form onSubmit={handleCreateTransaction} className="space-y-3">
              <div className="grid grid-cols-2 gap-2 p-1 bg-muted rounded-xl">
                <button
                  ...
                  className={`py-1.5 rounded-lg text-xs font-semibold transition cursor-pointer ${
                    type === 'expense' ? 'bg-[#FF5C39] text-white shadow-sm' : 'text-muted-foreground'
                  }`}
                >
                  Expense
                </button>
                <button
                  ...
                  className={`py-1.5 rounded-lg text-xs font-semibold transition cursor-pointer ${
                    type === 'income' ? 'bg-[#22C55E] text-white shadow-sm' : 'text-muted-foreground'
                  }`}
                >
                  Income
                </button>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <CustomSelect
                    value={category}
                    onChange={(val) => setCategory(val as TransactionItem['category'])}
                    options={[
                      { value: 'Salary', label: 'Salary' },
                      { value: 'Food', label: 'Food' },
                      { value: 'Transport', label: 'Transport' },
                      { value: 'Tech & Subscriptions', label: 'Tech & Subscriptions' },
                ...
                <div>
                  ...
                  <CustomSelect
                    value={paymentMethod}
                    onChange={(val) => setPaymentMethod(val as TransactionItem['paymentMethod'])}
                    options={[
