...
export function CalendarView() {
  ...
  const [category, setCategory] =
    useState<CalendarEvent["category"]>("deep-work");
  ...
  return (
    <ModuleContainer>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs sm:text-sm text-muted-foreground">
            Orchestrate your hours, meetings, and deep flow blocks in Day, Week,
            or Month view
          </p>
        </div>
      ...
      <div className="flex items-center justify-between p-4 rounded-3xl ios-card">
        ...
        <div className="hidden sm:flex items-center gap-3 text-[11px] font-semibold text-muted-foreground">
          <span className="flex items-center gap-1.5">
            <span className="w-2 h-2 rounded-full bg-[#8B5CF6]" /> Deep Work
          </span>
          <span className="flex items-center gap-1.5">
            <span className="w-2 h-2 rounded-full bg-[#0052FF]" /> Meeting
          </span>
          <span className="flex items-center gap-1.5">
            <span className="w-2 h-2 rounded-full bg-[#EC4899]" /> Fitness
          </span>
          <span className="flex items-center gap-1.5">
            <span className="w-2 h-2 rounded-full bg-[#22C55E]" /> Personal
          </span>
        </div>
      </div>
        ...
        <div className="p-4 sm:p-6 rounded-3xl ios-card">
          ...
          <div className="grid grid-cols-7 gap-1 sm:gap-2 pt-3">
            ...
            {[...Array(firstDayIndex)].map((_, i) => (
              <div
                key={`empty-${i}`}
                className="min-h-[48px] sm:min-h-[90px] rounded-xl sm:rounded-2xl bg-muted/20 border border-transparent"
              />
            ))}
        ...
        <div className="p-4 sm:p-6 rounded-3xl ios-card space-y-4">
          <div className="flex items-center justify-between">
            ...
            >
              <Plus className="w-3.5 h-3.5" />
              <span>
                Add to {formatDatePretty(selectedDayDate).split(",")[0]}
              </span>
            </Button>
          </div>
      ...
      <Modal
        ...
        title={formatDatePretty(selectedDayDate)}
        description={`${selectedDayEvents.length} ${
      ...
      >
        <div className="space-y-3 pt-1">
          {/* Event List */}
          <div className="space-y-2.5 overflow-y-auto max-h-[45vh] pr-1">
            {selectedDayEvents.length === 0 ? (
      ...
      <Modal
        ...
        title={`Add Event · ${formatDatePretty(eventDate)}`}
        description="Schedule a calendar block, sync meeting, or deep work sprint."
        ...
        <form onSubmit={handleCreateEvent} className="space-y-3.5 pt-1">
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-3.5 py-2.5 rounded-xl bg-muted/50 border border-border text-xs text-foreground focus:outline-none focus:ring-2 focus:ring-[#0052FF] transition resize-none"
            />
          </div>
