...
export function DesktopSidebar() {
  ...
  useEffect(() => {
    try {
      const saved =
        localStorage.getItem("trymonk_sidebar_collapsed") ||
        localStorage.getItem("aura_sidebar_collapsed");
      if (saved !== null) {
  ...
  return (
    <aside
      className={`hidden lg:flex flex-col shrink-0 h-screen sticky top-0 bg-sidebar text-sidebar-foreground border-r border-border overflow-y-auto transition-all duration-300 ease-in-out z-30 ${
        isCollapsed ? "w-[72px] px-2 py-4" : "w-64 px-3.5 py-5"
      }`}
    >
      {/* Brand Header with Unified Top Toggle */}
      <div
        className={`flex items-center mb-5 ${isCollapsed ? "justify-center" : "justify-between px-2"}`}
      >
        {!isCollapsed ? (
          <>
          ...
          <div className="space-y-1 pb-3 mb-2 border-b border-border/70">
            ...
            <div className="space-y-1">
              {userFavorites.map((favKey) => {
                ...
                const badge =
                  pageItem.key === "tasks" && pendingTasksCount > 0
                    ? String(pendingTasksCount)
                    : undefined;

                return (
                  <div
                    key={`fav-${pageItem.key}`}
                    className="relative group/fav"
                  >
                    <button
                      onClick={() => setActiveModule(pageItem.key)}
                      title={
                        isCollapsed ? `${pageItem.name} (Favorite)` : undefined
                      }
                    ...
                    >
                      <div className="flex items-center gap-2.5 min-w-0">
                        <Icon
                          className={`w-4 h-4 shrink-0 ${isActive ? "text-white" : "text-[#0052FF]"}`}
                        />
                        {!isCollapsed && (
                          <span
                            className={`truncate ${isActive ? "text-white" : "text-sidebar-foreground font-semibold"}`}
                          >
                            {pageItem.name}
                          </span>
                        )}
                      ...
                      {!isCollapsed && badge && (
                        <span
                          className={`text-[10px] font-bold px-2 py-0.5 rounded-full shrink-0 mr-4 group-hover/fav:mr-6 transition-all ${
                            isActive
                ...
                return (
                  <div key={item.key} className="relative group">
                    ...
                    >
                      <div className="flex items-center gap-2.5 min-w-0">
                        <Icon
                          className={`w-4 h-4 shrink-0 ${isActive ? "text-white" : "text-muted-foreground group-hover:text-sidebar-foreground"}`}
                        />
                        {!isCollapsed && (
                          <span
                            className={`truncate ${isActive ? "text-white" : "text-sidebar-foreground"}`}
                          >
                            {item.name}
                          </span>
                        )}
                      ...
                      {!isCollapsed && badge && (
                        <span
                          className={`text-[10px] font-bold px-2 py-0.5 rounded-full shrink-0 mr-4 group-hover:mr-6 transition-all ${
                            isActive
                              ? "bg-white/20 text-white"
                              : "bg-muted text-foreground"
                          }`}
                    ...
                    {!isCollapsed && (
                      <div className="absolute right-1.5 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition">
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent
                            align="end"
                            className="w-44 text-xs space-y-1"
                          >
                            <DropdownMenuItem
                              onClick={(e) => {
                                e.stopPropagation();
                            ...
                            >
                              <Star
                                className={`w-3.5 h-3.5 ${isFavorited ? "text-[#0052FF] fill-[#0052FF]" : "text-muted-foreground"}`}
                              />
                              <span>
                                {isFavorited
            ...
            <div className="flex items-center gap-2.5 overflow-hidden">
              ...
              <div className="overflow-hidden">
                ...
                <span className="text-[10px] text-muted-foreground flex items-center gap-1">
                  <Flame className="w-3 h-3 text-[#0052FF] inline" />{" "}
                  {user.streak || 0}d streak • Lvl {user.level || 1}
                </span>
              </div>
            </div>
