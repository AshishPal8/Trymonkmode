...
export function QuickAddModal() {
  ...
  const [activeTab, setActiveTab] = useState<
    "task" | "note" | "habit" | "event" | "expense" | "goal" | "bookmark"
  >("task");
  ...
  const [habitCategory, setHabitCategory] = useState<
    "Fitness" | "Health" | "Mindset" | "Learning" | "Work"
  >("Health");
  ...
  const [eventCategory, setEventCategory] = useState<
    "deep-work" | "meeting" | "fitness" | "personal" | "learning"
  >("deep-work");
  ...
  const [goalCategory, setGoalCategory] =
    useState<GoalItem["category"]>("Career");
  const [goalTimeframe, setGoalTimeframe] =
    useState<GoalItem["timeframe"]>("yearly");
  ...
  const [bmCategory, setBmCategory] = useState<
    "Websites" | "YouTube" | "Articles" | "GitHub" | "Courses" | "Books"
  >("Websites");
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (activeTab === "task" && taskTitle.trim()) {
      addTask({
        ...
        completed: false,
        quadrant:
          taskPriority === "P1" ? "urgent-important" : "notUrgent-important",
      });
      setTaskTitle("");
      setTaskDesc("");
      ...
      addBookmark({
        title: bmTitle.trim(),
        url: bmUrl.trim().startsWith("http")
          ? bmUrl.trim()
          : `https://${bmUrl.trim()}`,
        category: bmCategory,
        type: "link",
  ...
  return (
    <Modal
      ...
      title="Quick Create"
      description="Create tasks, notes, habits, events, or resources in seconds."
      icon={<Sparkles className="w-4 h-4" />}
      topAccentColor="#0052FF"
      maxWidth="lg"
    >
      {/* Tab Selectors */}
      <div className="flex items-center gap-1.5 pt-1 overflow-x-auto pb-2 border-b border-border/60">
        {[
          ...
          return (
            <Button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs whitespace-nowrap transition cursor-pointer ${
                isActive
      ...
      <form
        ...
        className="pt-4 space-y-4 max-h-[65vh] overflow-y-auto px-1.5 py-1"
      >
        {/* TASK TAB */}
        {activeTab === "task" && (
          <>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <CustomSelect
                  value={taskPriority}
                  onChange={(val) => setTaskPriority(val as PriorityLevel)}
                  options={[
                    { value: "P1", label: "P1 - Urgent & Important" },
                    { value: "P2", label: "P2 - Important Focus" },
                    { value: "P3", label: "P3 - Urgent Delegation" },
              ...
              <div>
                ...
                <CustomDatePicker
                  value={taskDueDate}
                  onChange={(val) =>
                    setTaskDueDate(val || getTodayDateString())
                  }
                />
              </div>
            </div>
          ...
          <>
            <div>
              ...
              <Input
                ...
                placeholder="e.g. Architecture Principles"
              />
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Write your note, checklist, or instructions..."
                className="w-full px-3.5 py-2.5 rounded-xl bg-muted/60 border border-border text-xs sm:text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-[#0052FF] resize-none"
              />
            </div>
            ...
            <div>
              ...
              <div className="flex items-center justify-between p-2 rounded-2xl bg-muted/40 border border-border">
                {NOTE_COLORS.map((c) => {
                  const isSelected = noteColor === c.hex;
                  return (
                    <button
                      key={c.hex}
                      type="button"
          ...
          <>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <CustomSelect
                  value={habitCategory}
                  onChange={(val) => setHabitCategory(val as any)}
                  options={[
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <CustomSelect
                  value={goalCategory}
                  onChange={(val) =>
                    setGoalCategory(val as GoalItem["category"])
                  }
                  options={[
              ...
              <div>
                ...
                <CustomSelect
                  value={goalTimeframe}
                  onChange={(val) =>
                    setGoalTimeframe(val as GoalItem["timeframe"])
                  }
                  options={[
