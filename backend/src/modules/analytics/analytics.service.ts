...
const CATEGORY_COLORS = ['#0052FF', '#8B5CF6', '#F59E0B', '#10B981', '#EC4899', '#06B6D4', '#6366F1'];

export async function getAnalyticsService(userId: number): Promise<AnalyticsSummary> {
  // 1. Fetch User Data
  ...
  // 3. Generate past 7 days (including today)
  ...
  // 4. Map activity across the past 7 days
  const focusPerTaskMinutes = settings?.focusDuration || 25;
  let maxDayScore = 0;

  const dailyVelocity: DailyVelocity[] = past7Days.map(({ dateStr, dayName }) => {
    const tasksDoneOnDay = userTasks.filter(
      t => t.completed && String(t.updatedAt || t.createdAt).slice(0, 10) === dateStr
    ).length;
    ...
    const estimatedFocusHours = Number(((tasksDoneOnDay * focusPerTaskMinutes + habitsDoneOnDay * 15) / 60).toFixed(1));
    const dayScore = tasksDoneOnDay * 2 + habitsDoneOnDay + estimatedFocusHours;
  ...
  // Calculate height percentages and identify peak day
  ...
  dailyVelocity.forEach(d => {
    ...
    d.heightPct = maxDayScore > 0 ? Math.max(10, Math.round((score / maxDayScore) * 100)) : 0;
  });

  const totalFocusHours = Number(dailyVelocity.reduce((acc, curr) => acc + curr.focusHours, 0).toFixed(1));

  // 5. Weekly Habit Adherence & Completion Rates
  ...
  const weeklyAdherenceRate = totalHabitOpportunities > 0
    ? Math.round((totalHabitsCompletedThisWeek / totalHabitOpportunities) * 100)
  ...
  // 6. Optimal Biological Time Window
  let morningTasks = 0;
  ...
  let optimalTimeDescription = 'Plan deep work during morning hours to maximize uninterrupted flow state.';
  ...
  if (hasActivity) {
    ...
    optimalTimeDescription = 'Complete tasks and start focus sessions to calibrate your personalized biological peak window.';
  }

  // 7. Dynamic Velocity Tier & Flow State Tier
  const streak = user?.streak || 0;
  const velocityTier = weeklyAdherenceRate >= 90 && streak >= 7
  ...
  // 8. Focus Session & Task Category Distribution
  ...
  userTasks.forEach(t => {
    ...
    if (rawTags.length > 0) {
      ...
      const q = t.quadrant === 'urgent-important' ? 'Critical Priorities' : 'Core Projects';
  ...
  const totalCategoryPoints = Object.values(categoryCounts).reduce((a, b) => a + b, 0);
  const categoryDistribution: CategoryDistributionItem[] = Object.entries(categoryCounts)
    .map(([tag, count], index) => ({
      tag,
      count,
      pct: totalCategoryPoints > 0 ? Math.round((count / totalCategoryPoints) * 100) : 0,
